...
ndf = pd.DataFrame({p0:np.log(nutrient_demand(reference_x,ceteris_paribus_price(USE_GOOD,p0)))[UseNutrients] for p0 in P}).T
...
log_nar = {s0:np.log(nutrient_adequacy_ratio(poorer_x,ceteris_paribus_price(USE_GOOD,s0,p)))[UseNutrients] for s0 in Pscale}
